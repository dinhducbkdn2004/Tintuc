...
<body>
	<nav class="w-full h-8 bg-[#ed1b24] text-white">
		<div
			...
			<a href="<%=path%>" target="_parent"> <span
				class="uppercase font-bold text-xs px-2 hover:text-amber-100 translate-y-[-1px]">
					<span class="inline-block mr-1 translate-y-[2px]"> <i
						class="ph-fill ph-house text-sm"></i>
				</span> Trang chủ
			</span>
			</a> <a href="<%=path%>/articles?subject=thoi-su" target="_parent">
				<span class="hover:text-amber-100">Thời sự</span>
			</a> <a href="<%=path%>/articles?subject=the-gioi" target="_parent">
				<span class="hover:text-amber-100">Thế giới</span>
			</a> <a href="<%=path%>/articles?subject=kinh-te" target="_parent">
				<span class="hover:text-amber-100">Kinh tế</span>
			</a> <a href="<%=path%>/articles?subject=xa-hoi" target="_parent"> <span
				class="hover:text-amber-100">Xã hội</span>
			</a> <a href="<%=path%>/articles?subject=phap-luat" target="_parent">
				<span class="hover:text-amber-100">Pháp luật</span>
			</a> <a href="<%=path%>/articles?subject=van-hoa" target="_parent">
				<span class="hover:text-amber-100">Văn hóa</span>
			</a> <a href="<%=path%>/articles?subject=giao-duc" target="_parent">
				<span class="hover:text-amber-100">Giáo dục</span>
			</a> <a href="<%=path%>/articles?subject=the-thao" target="_parent">
				<span class="hover:text-amber-100">Thể thao</span>
			</a> <a href="<%=path%>/articles?subject=quan-su" target="_parent">
				<span class="hover:text-amber-100">Quân sự</span>
			</a> <a href="<%=path%>/articles?subject=khcn" target="_parent">
				<span class="hover:text-amber-100">Khoa học - Công nghệ</span>
			</a> <a href="<%=path%>/articles?subject=y-te" target="_parent"> <span
				class="hover:text-amber-100">Y tế</span>
			</a>
			<%
			if (user != null && user.getUserRole() == User.Role.admin) {
			%>
			<div class="h-6 w-[1px] bg-white/50 mx-1"></div>
			<a href="<%=path%>/manage" target="_parent"> <span
				class="uppercase font-bold text-xs px-2 hover:text-amber-100 translate-y-[-1px]">
					<span class="inline-block mr-1 translate-y-[2px]"> <i
						class="ph ph-gear text-sm"></i>
				</span> Trang quản lý
			</span>
			</a>
			<%
