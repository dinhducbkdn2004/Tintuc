...
<body>
	<iframe class="w-full h-[110px]" src="<%=path%>/views/header.jsp"
		frameborder="0" loading="eager"></iframe>
	<iframe class="w-full h-8 sticky top-[-1px] z-10"
		src="<%=path%>/views/navigation.jsp" frameborder="0" loading="eager"></iframe>
	<main class="py-6">
		<div class="main-wrapper flex gap-4">
			<section id="home-articles" class="flex-1">
				<div
					...
					<h1 class="uppercase font-semibold text-[#d21d21]">
						Tin <%=subjectName%>
					</h1>
				...
				<div class="w-full flex flex-col gap-4 mt-2">
					...
					<div class="flex-center px-4 py-4">
						<a
							href="<%=path%>/articles?subject=<%=subjectId%>&page=<%=pageNumber + 1%>"
							class="font-semibold bg-[#d21d21]/90 hover:bg-[#d21d21] text-white px-4 py-1.5 transition-colors">Xem
							thêm</a>
					</div>
					<%
					}
